...
import { generateMockPayoutCreated, generateMockPayoutHistory } from '../../../lib/mockdata';
  ...
  try {
    ...
    if (!orderId || !recipientId || !recipientType || amount === undefined) {
      return NextResponse.json(
        { error: 'Faltan campos obligatorios: orderId, recipientId, recipientType o amount.' },
        { status: 400 }
      );
    }
    ...
    // =====================================================================
    // const response = await fetch('https://api.payments-app.com/api/payments/payouts', {
    //   method: 'POST',
...
export async function GET(request: Request) {
  try {
    ...
    if (!recipientId || !recipientType) {
      return NextResponse.json(
        { error: 'Faltan los parámetros de consulta: recipientId o recipientType.' },
        { status: 400 }
      );
    }
    ...
    // IMPLEMENTACIÓN FUTURA (Llamada real a la aplicación de Payments)
    // =====================================================================
    // const response = await fetch(`https://api.payments-app.com/api/payments/payouts?recipientId=${recipientId}&recipientType=${recipientType}`);
    ...
    const mockPayouts = generateMockPayoutHistory(recipientId, recipientType);
